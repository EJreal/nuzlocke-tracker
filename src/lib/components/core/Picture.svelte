<script>
  export let src, alt = '', className = '', aspect = '4x4', pixelated = false, role = '', fadeIn = true, loading = 'lazy', webp = true
...
<picture class={$$restProps.class || ''} in:fade>
  ...
  {:else}
  <img class='{className}' class:pixelated={pixelated} src='{src}.png' {loading} {width} {height} {alt} {role} onerror='this.onerror = null;'/>
  {/if}
</picture>
{:else}
